data "azurerm_cosmosdb_account" "pipeline_metrics" {
  ...
  name                = var.env == "sbox" ? "sandbox-pipeline-metrics" : "pipeline-metrics"
  resource_group_name = var.env == "sbox" ? "pipelinemetrics-database-sandbox" : "pipelinemetrics-database-prod"
}
